  ...
  return (
    <div className="max-w-md mx-auto px-4 py-10">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <button type="submit" className="w-full bg-primary text-white rounded px-4 py-2" disabled={loading}>
          {loading ? 'Logging in...' : 'Login'}
        </button>
      </form>
      <div className="mt-4 text-sm text-gray-700">Don't have an account? <Link to="/register" className="text-primary hover:underline">Sign up</Link></div>
      {loading && <Loader />}
    </div>
  );
}
